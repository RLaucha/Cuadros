// =================== CONFIG ===================
const BUSINESS = {
  whatsapp: "+5491167214914", // <-- poné tu número real (+54...)
  brand: "Aura Deco",
  city: "Buenos Aires",
};
...
// Productos (podés reemplazar img por URLs o por rutas locales en /img)
const PRODUCTS = [
...
// 3.3 Mensaje de WhatsApp más claro en checkout
checkoutBtn?.addEventListener("click", ()=>{
  ...
  const msg = [`Hola! Quiero armar pedido de *Aura Deco*:`, ...lines, `Subtotal: ${total}`, "", "¿Disponibilidad y tiempos?"].join("\n");
...
// 3.2 Cerrar <dialog> con click en fondo y con ESC
["#productModal", "#cartDrawer"].forEach(sel=>{
...
// 3.4 Datos estructurados de productos (opcional)
function injectSchema(){
  ...
  const itemList = PRODUCTS.map((p,i)=>({
    ...
    "description": p.desc || "",
    "sku": String(p.id),
    "brand": {"@type":"Brand","name":"Aura Deco"},
    "offers": {"@type":"Offer","priceCurrency":"ARS","price": p.price,"availability":"http://schema.org/InStock"},
    "position": i+1
  }));
  const s = document.createElement("script");
  s.type="application/ld+json";
  s.textContent = JSON.stringify({"@context":"https://schema.org","@type":"ItemList","itemListElement": itemList});
  document.head.appendChild(s);
}
document.addEventListener("DOMContentLoaded", ()=>{ try{injectSchema()}catch{} });